...
class CodeInterpreterAgent:
    """
    AI Agent for interpreting code.
    ...
    def ask_ai(
        ...
        try:
            ...
            assistant = self.ai_service.get_ai_client().beta.assistants.create(
                ...
                instructions="You are an expert excel tutor. When asked to modify an Excel file, write and run code to answer the question. You can return the modified file.",
                model=self.model,
                tools=[{"type": "code_interpreter"}],
                tool_resources={"code_interpreter": {"file_ids": [message_file.id]}},
            )
            ...
            run_steps = self.ai_service.get_ai_client().beta.threads.runs.steps.list(
                thread_id=thread.id,
                run_id=run.id
            )
            print(run_steps)
            ...
            ai_response = self.ai_service.ask_ai(
                ...
                ai_analytics_agent_name="CodeInterpreterAgent",
            ...
            logging.error(f"Erro ao comunicar com o AI CodeInterpreterAgent: {e}")
            raise
    ...
    def do_your_work_with(
        ...
        try:
            ...
            logging.error(f"AI CodeInterpreterAgent: Error communicating with AI: {e}")
            raise

        try:
            response_json = json.loads(reponse)
        except json.JSONDecodeError:
            logging.error(f"AI CodeInterpreterAgent: Error parsing AI response JSON: {reponse}")
            raise
        ...
        try:
            ...
            logging.error(f"AI CodeInterpreterAgent: Error parsing AI response JSON arguments: {response_json['function']['arguments']}")
            raise
